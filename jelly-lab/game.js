...
renderer.setClearColor(0xffe8a0, 1);
...
function buildWorld() {
  ...
  const peep = new THREE.Mesh(new THREE.CircleGeometry(0.06, 16), matColor(0x111));

  peep.position.set(DH, 1.58, 0.07);

  peep.userData.maxDist = 1.25;
...
function doRayInteract(nx, ny) {
  ...
  for (const h of hits) {
    ...
    const id = h.object?.userData?.clickId;

    if (!id || h.distance > INTERACT_MAX) continue;

    const cap = h.object.userData.maxDist;

    if (typeof cap === "number" && h.distance > cap) continue;

    if (id === "peep") {

      // глазок только в упор — иначе всегда «ворует» клик с двери/комнаты

      if (h.distance > 1.25) continue;
  ...
  if (best?.object?.userData?.clickId) {

    bumpInteract();

    interact(best.object.userData.clickId, best.object);
...
try {
  ...
  errBox.id = "sceneErr";
  errBox.textContent = "Комната не загрузилась. Нажми Ctrl+F5 или открой с ?v=22";
  document.body.appendChild(errBox);
  say("Что-то сломалось. Обнови страницу: Ctrl+F5.");
}
renderInv();
resize();
updateCam();
