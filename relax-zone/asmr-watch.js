 ...
 * ASMR «видео» v3 — мягкий squish, нежный лёд (не режет уши).
...
(function () {
  ...
  var audioCtx, master, softFilter;
  ...
  var VIDEOS = [

    {
      ...
      sub: "Мягкий squish · тихий лёд · жми экран",
...

    {
      ...
      sub: "Тихий squish · жми если хочешь",
  ...
  function ensureAudio() {
    ...
    softFilter.connect(audioCtx.destination);
  ...
  function softVol() {
    ...
    var base = s ? Number(s.value) / 100 : 0.42;

    return Math.min(0.75, base * 0.85);
  ...
  function noiseBurst(dur, filterFreq, gain, type, q) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      d[i] = (Math.random() * 2 - 1) * env * env * env;
  ...
  function toneAt(t, freq, dur, type, gain, sweep) {
    ...
    gn.gain.exponentialRampToValueAtTime(Math.max(0.001, gain * vol()), t + 0.012);
  ...
  /** Мягкий баттерсквиш — низкие частоты, без резкости */
  ...
  function butterSquish(extraLoud) {
    ...
    var amp = extraLoud ? 0.75 : 0.55;

    toneAt(t, 95 + Math.random() * 15, 0.2, "sine", 0.28 * amp, 70);

    toneAt(t + 0.03, 62, 0.28, "sine", 0.22 * amp, 88);

    noiseBurst(0.07, 380, 0.18 * amp, "lowpass", 0.6);

    setTimeout(function () {

      toneAt(audioCtx.currentTime, 78, 0.32, "sine", 0.18 * amp, 105);

      noiseBurst(0.06, 320, 0.12 * amp, "lowpass", 0.5);
  ...
  /** Нежный лёд — тихий crack, без высоких частот */

  function iceCrunch(extraLoud) {

    resumeAudio();

    var amp = extraLoud ? 0.7 : 0.5;

    noiseBurst(0.05, 900, 0.22 * amp, "bandpass", 0.8);

    noiseBurst(0.04, 520, 0.16 * amp, "lowpass", 0.6);

    var t = audioCtx.currentTime;

    toneAt(t, 280 + Math.random() * 80, 0.04, "sine", 0.08 * amp);

    setTimeout(function () {

      noiseBurst(0.03, 650, 0.1 * amp, "lowpass", 0.5);
  ...
  /** Баттер + лёд — мягко, с паузой */

  function butterIceASMR(extraLoud) {

    butterSquish(extraLoud);

    setTimeout(function () { iceCrunch(false); }, 220);
  ...
  function splash() {
    ...
    noiseBurst(0.08, 450, 0.28, "lowpass", 0.6);

    noiseBurst(0.05, 220, 0.18, "lowpass", 0.4);
  ...
  function rainTick() {
    ...
    noiseBurst(0.02, 1400, 0.12, "bandpass", 1.5);
  ...
  function maybeAutoSound(elapsed, videoId) {
    ...
    if (videoId === "butter_ice") {
      ...
      if (press > 0.92 && lastPressPhase <= 0.92) butterSquish(false);

      if (press < -0.85 && lastPressPhase >= -0.85) butterSquish(false);

      lastPressPhase = press;

    } else if (videoId === "ice_water") {

      if (elapsed - lastAutoAt > 4) {
  ...
  function drawButterIce(elapsed) {
    ...
    ctx.fillText("🔊 жми — мягкий squish + лёд", cx, H - 24);
  ...
  function drawSquish(elapsed) {
    ...
    ctx.fillText("🔊 тихий squish · жми если хочешь", W / 2, H - 28);
  ...
  function drawIceWater(elapsed) {
    ...
    ctx.fillText("🔊 жми — нежный хруст", W / 2, H - 28);
  ...
  function init() {
    ...
    volEl.oninput = function () {
      ...
      if (master) master.gain.value = softVol();
